...
Encoder e1(0,255,2,4,8,&encoderInterruptHandler1);   // constructor 6 parameters     
                                              //params: minimum, maximum, Clock pin, DT pin, SW pin, nameHandlerFunction
void setup() {
  ...
  e1.setPosition(128);                         // 0=0% dutycicle, 128=50% dutycycle, 255=100% dutycicle
  e1.setSwitchInit(HIGH);                      // set initial value in HIGH 
}

void loop() {
  analogWrite(PWM,e1.getPosition());           // update dutycycle 
  digitalWrite(LED,e1.getSwitchToggle());      // update LED status
}

void encoderInterruptHandler1() {             // interrupt attending for encoder
